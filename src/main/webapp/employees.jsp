		...
		<div class="p-8">
			...
			<div>
				<div class="container mx-auto">
					<h1 class="text-3xl font-bold mb-6 text-gray-900">Employees
						Management</h1>
						...
						<table class="min-w-full text-left text-sm font-light">
							...
							<tbody>
								...
								if (searchQuery != null && !searchQuery.trim().isEmpty()) {
									...
									"SELECT * FROM users WHERE role='employee' AND (name LIKE ? OR email LIKE ? OR phone LIKE ?)");
									String query = "%" + searchQuery + "%";
									ps.setString(1, query);
									ps.setString(2, query);
									ps.setString(3, query);
								} else {
									ps = conn.prepareStatement("SELECT * FROM users WHERE role='employee'");
								}
		...
		: "text-red-600"%> font-semibold">
									...
									<td class="px-6 py-4 text-right">
										<div class="inline-flex rounded-md shadow-xs" role="group">
											<a
												href="employeeDetails.jsp?id=<%=rs.getInt("id")%>"
												...
												View </a> <a href="deleteEmployee.jsp?id=<%=rs.getInt("id")%>"
